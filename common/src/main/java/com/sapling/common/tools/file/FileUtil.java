...
public class FileUtil {
    ...
    public static String getParentDir(String src, String fileSeperator) {
        ...
        if (src.endsWith(fileSeperator)) {
            index = src.substring(0,
                    src.length() - 1).lastIndexOf(fileSeperator);
        } else {
    ...
    public static File createFile(String fileName, byte[] bytes, boolean isDelete) {
        File tmpFile = null;
        FileOutputStream fileOutputStream = null;
        try {
            tmpFile = new File(fileName);
    ...
    public static String pathToPackage(String pathName) {
        ...
        if (pathName.indexOf(File.separator) == -1 &&
                pathName.indexOf(FileUtil.FILE_SEPERATOR_UNIX) == -1) {
            return pathName;
        }
    ...
    public static void appendToFile(File file, String content, String charset) throws FileNotFoundException {
